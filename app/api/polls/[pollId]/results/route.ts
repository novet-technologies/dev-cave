...
export async function POST(
  ...
  try {
    ...
    const totalResponses = poll.options?.reduce((sum, opt) => sum + (opt.responses?.length || 0), 0) || 0
    const allResponded = totalResponses >= totalMembers

    if (!isAdmin && !allResponded) {
      return NextResponse.json({ error: 'Not authorized or not all members responded' }, { status: 403 })
    }
    ...
    const pollData = {
      ...
      options: poll.options?.map(opt => ({
        ...
        voters: opt.responses?.map(r => r.user?.display_name || r.user?.username).join(', ')
      })) || [],
      totalResponses,
      totalMembers
    }
    ...
    try {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful assistant that analyzes poll results and provides concise insights. Keep responses under 200 words and focus on key trends and interesting findings.'
          },
          {
            role: 'user',
...
Results:
${pollData.options.map(opt => `- ${opt.text}: ${opt.votes} votes (${opt.voters})`).join('\n')}
